...
class BaseDatos:
    ...
    def agregar_peticion(self, ip, tipo, contenido, detalles):
        with self.lock:
            ...
            self.conn.commit()
        self.log_evento('INFO', 'Peticion de ' + ip + ': ' + contenido)
    ...
    def registrar_pago(self, ip, concepto, monto, notas=""):
        with self.lock:
            ...
            self.conn.commit()
        self.log_evento('INFO', 'Pago: ' + str(monto) + ' CUP de ' + ip)
    ...
    def backup_db(self):
        ...
        try:
            ...
            self.log_evento('INFO', 'Backup: ' + destino)
            return destino
    ...
    def exportar_csv(self, tipo):
        with self.lock:
            ...
            if tipo == 'descargas':
                ...
                w.writerow(['IP','Dispositivo','Primera','Ultima','Visitas','Bloqueado'])
...
.hd::after{content:'';position:absolute;bottom:0;right:-100%;width:100%;height:1px;background:linear-gradient(90deg,transparent,var(--a),transparent);animation:sc2 5s linear infinite}
@keyframes sc{0%{left:-100%}100%{left:100%}}
@keyframes sc2{0%{right:-100%}100%{right:100%}}
...
.bt{padding:11px 22px;border-radius:10px;border:none;font-weight:600;font-size:14px;cursor:pointer;transition:all 0.3s;font-family:'Courier New',monospace;position:relative;overflow:hidden}
.bt::after{content:'';position:absolute;top:50%;left:50%;width:0;height:0;background:rgba(255,255,255,0.15);border-radius:50%;transform:translate(-50%,-50%);transition:width 0.5s,height 0.5s}
.bt:active::after{width:200px;height:200px}
...
.bg2:hover{background:rgba(0,255,136,0.15);border-color:var(--p);transform:translateY(-1px)}
...
.stt::before{content:'>';color:var(--p);animation:bl 1s infinite;font-weight:400}
...
.csl img{width:170px;height:230px;object-fit:cover;border-radius:14px;border:2px solid rgba(0,255,136,0.4);box-shadow:0 8px 40px rgba(0,0,0,0.5),0 0 30px rgba(0,255,136,0.15);transition:all 0.4s;flex-shrink:0}
.csl img:hover{transform:scale(1.04) rotate(1deg);box-shadow:0 12px 50px rgba(0,0,0,0.6),0 0 50px rgba(0,255,136,0.25)}
...
.sc:hover{transform:translateY(-4px);border-color:rgba(0,255,136,0.3);box-shadow:0 8px 30px rgba(0,255,136,0.1)}
...
.flt:hover{border-color:rgba(0,255,136,0.3);color:var(--p)}
.flt.act{background:rgba(0,255,136,0.15);border-color:rgba(0,255,136,0.4);color:var(--p);box-shadow:0 2px 10px rgba(0,255,136,0.1)}
.cp{margin:6px 0}
...
.cpt:hover{background:rgba(0,255,136,0.12);border-color:rgba(0,255,136,0.3);transform:translateX(4px)}
...
.vt button:hover{background:rgba(0,255,136,0.15);border-color:var(--p)}
.vt button:disabled{opacity:0.4;cursor:default}
.anc{background:rgba(0,212,255,0.06);border:1px solid rgba(0,212,255,0.2);border-radius:12px;padding:16px 20px;margin-bottom:12px;border-left:4px solid var(--s);transition:all 0.3s}
.anc:hover{border-color:rgba(0,212,255,0.4);transform:translateX(4px)}
...
.md.ac{display:flex}
.mdc{background:var(--g);border:1px solid rgba(0,255,136,0.3);border-radius:18px;padding:36px;max-width:480px;width:100%;box-shadow:0 20px 80px rgba(0,0,0,0.6),0 0 40px rgba(0,255,136,0.1);position:relative;max-height:90vh;overflow-y:auto}
...
.fg input,.fg select,.fg textarea{width:100%;padding:11px 14px;background:rgba(0,0,0,0.4);border:1px solid rgba(0,255,136,0.2);border-radius:10px;color:var(--l);font-family:'Courier New',monospace;font-size:14px;transition:all 0.2s}
.fg input:focus,.fg select:focus,.fg textarea:focus{border-color:var(--p);box-shadow:0 0 20px rgba(0,255,136,0.15);outline:none}
...
.toast{position:fixed;bottom:24px;right:24px;background:var(--g);backdrop-filter:blur(15px);border:1px solid var(--p);border-radius:12px;padding:14px 22px;font-family:'Courier New',monospace;font-size:13px;color:var(--p);z-index:9999;box-shadow:0 8px 30px rgba(0,0,0,0.5);transform:translateY(100px);opacity:0;transition:all 0.4s cubic-bezier(0.4,0,0.2,1)}
.toast.show{transform:translateY(0);opacity:1}
.toast.err{border-color:var(--d);color:var(--d)}
::-webkit-scrollbar{width:8px}::-webkit-scrollbar-track{background:var(--dk)}::-webkit-scrollbar-thumb{background:rgba(0,255,136,0.4);border-radius:10px}::-webkit-scrollbar-thumb:hover{background:var(--p)}
...
<p style="color:rgba(224,230,237,0.6);margin-bottom:20px;font-family:'Courier New',monospace;font-size:13px">No encuentras lo que buscas? Dinoslo y lo agregaremos pronto.</p>
<form id="fPet">
...
<div class="fg"><label>detalles</label><textarea name="detalles" placeholder="temporada, episodio, calidad preferida..."></textarea></div>
...
var pts=[];for(i=0;i<30;i++){pts.push({x:Math.random()*pc.width,y:Math.random()*pc.height,r:Math.random()*2+1,dx:(Math.random()-0.5)*0.5,dy:(Math.random()-0.5)*0.5});}
...
# ============================================================
# HTML PANEL ADMIN (CORREGIDO SESION + MEJORADO)
# ============================================================
...
.nav button:hover{color:var(--p);background:rgba(0,255,136,0.06)}
.nav button.act{background:rgba(0,255,136,0.12);border-color:rgba(0,255,136,0.3);color:var(--p);box-shadow:0 2px 10px rgba(0,255,136,0.1)}
...
.sc:hover{border-color:rgba(0,255,136,0.3);transform:translateY(-2px);box-shadow:0 6px 20px rgba(0,0,0,0.3)}
...
.sv{font-size:22px;font-weight:800;color:var(--p);font-family:'Courier New',monospace;text-shadow:0 0 15px rgba(0,255,136,0.3)}
.sl2{font-size:10px;color:rgba(224,230,237,0.5);margin-top:4px;font-family:'Courier New',monospace;text-transform:uppercase;letter-spacing:0.5px}
...
.cd h2{font-size:16px;margin-bottom:14px;color:var(--p);font-family:'Courier New',monospace;display:flex;align-items:center;gap:8px}
...
th{color:rgba(0,255,136,0.7);font-weight:600;text-transform:uppercase;font-size:10px;letter-spacing:0.5px;background:rgba(0,255,136,0.03)}
td{color:var(--l)}
...
.fg label{display:block;margin-bottom:5px;color:rgba(224,230,237,0.6);font-size:11px;font-weight:600;font-family:'Courier New',monospace;text-transform:uppercase;letter-spacing:0.5px}
.fg input,.fg select,.fg textarea{width:100%;padding:9px 12px;background:rgba(0,0,0,0.4);border:1px solid rgba(0,255,136,0.2);border-radius:8px;color:var(--l);font-family:'Courier New',monospace;font-size:13px;transition:all 0.2s}
.fg input:focus,.fg select:focus,.fg textarea:focus{border-color:var(--p);box-shadow:0 0 15px rgba(0,255,136,0.15);outline:none}
.fg textarea{resize:vertical;min-height:60px}
.tbs{display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap}
.tb{padding:6px 14px;border-radius:6px;background:rgba(0,255,136,0.06);border:1px solid rgba(0,255,136,0.15);cursor:pointer;font-size:11px;font-family:'Courier New',monospace;color:rgba(224,230,237,0.7);transition:all 0.2s}
.tb:hover{border-color:rgba(0,255,136,0.3);color:var(--p)}
.tb.ac{background:rgba(0,255,136,0.15);border-color:rgba(0,255,136,0.4);color:var(--p)}
.anc{background:rgba(0,212,255,0.06);border:1px solid rgba(0,212,255,0.2);border-radius:8px;padding:12px 16px;margin-bottom:10px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px}
.anc strong{color:var(--s);font-family:'Courier New',monospace;font-size:13px}
.anc small{color:rgba(224,230,237,0.5);display:block;margin-top:4px}
...
.toast.show{transform:translateY(0);opacity:1}
.toast.err{border-color:var(--d);color:var(--d)}
@media(max-width:768px){.sg{grid-template-columns:repeat(2,1fr)}.hd{padding:16px}.nav{gap:4px}.nav button{padding:6px 10px;font-size:11px}}
@media(max-width:480px){.sg{grid-template-columns:1fr 1fr}.sc{padding:14px 10px}.sv{font-size:18px}}
</style>
</head>
<body>
<canvas id="mc"></canvas>
...
<div class="cd"><h2>📊 actividad_7_dias.log</h2><div style="position:relative;height:260px"><canvas id="chA"></canvas></div><div id="chF" style="display:none;text-align:center;padding:40px;color:rgba(224,230,237,0.4);font-family:'Courier New',monospace;font-size:13px">⚠️ Chart.js no cargado - los datos estan en las tablas</div></div>
...
mostrarToast('⚠️ sesion expirada, inicia sesion de nuevo',true);
...
function cargarPagos(){fetch('/api/admin/pagos?token='+encodeURIComponent(tok)).then(function(r){return r.json()}).then(function(d){var h='';for(var i=0;i<d.length;i++){h+='<tr><td>'+d[i][1]+'</td><td>'+d[i][2]+'</td><td style="color:var(--p);font-weight:bold">'+d[i][3]+' CUP</td><td>'+d[i][4]+'</td><td>'+(d[i][5]||'-')+'</td></tr>';}document.querySelector('#tPagos tbody').innerHTML=h||'<tr><td colspan="5" style="text-align:center;color:rgba(224,230,237,0.4)">sin pagos</td></tr>';}).catch(function(){});}
function cargarCodigos(){fetch('/api/admin/codigos?token='+encodeURIComponent(tok)).then(function(r){return r.json()}).then(function(d){var h='';for(var i=0;i<d.length;i++){h+='<tr><td style="color:var(--p);font-weight:bold;font-size:13px">'+d[i][1]+'</td><td>'+d[i][2]+'</td><td>'+d[i][3]+'</td><td>'+(d[i][4]===1?'<span class="sr">usado</span>':'<span class="sa">disponible</span>')+'</td><td>'+d[i][5]+'</td></tr>';}document.querySelector('#tCodigos tbody').innerHTML=h||'<tr><td colspan="5" style="text-align:center;color:rgba(224,230,237,0.4)">sin codigos</td></tr>';}).catch(function(){});}
function cargarAnunciosAdmin(){fetch('/api/admin/anuncios?token='+encodeURIComponent(tok)).then(function(r){return r.json()}).then(function(d){var h='';for(var i=0;i<d.length;i++){h+='<div class="anc"><div><strong>'+d[i][1]+'</strong><small>'+d[i][2]+'</small></div><button class="ab2 '+(d[i][3]===1?'bd':'bs2')+'" data-aid="'+d[i][0]+'" data-acc="toggleAnc">'+(d[i][3]===1?'desactivar':'activar')+'</button></div>';}document.getElementById('listaAnc').innerHTML=h||'<p style="color:rgba(224,230,237,0.4);font-family:Courier New,monospace;font-size:13px">sin anuncios</p>';}).catch(function(){});}
function cargarLogs(){fetch('/api/admin/logs?token='+encodeURIComponent(tok)).then(function(r){return r.json()}).then(function(d){var h='';for(var i=0;i<d.length;i++){var cls=d[i][1]==='ERROR'?'sr':(d[i][1]==='WARN'?'sp':'scc');h+='<tr><td><span class="'+cls+'">'+d[i][1]+'</span></td><td>'+d[i][2]+'</td><td style="color:rgba(224,230,237,0.4)">'+d[i][3]+'</td></tr>';}document.querySelector('#tLogs tbody').innerHTML=h||'<tr><td colspan="3" style="text-align:center;color:rgba(224,230,237,0.4)">sin logs</td></tr>';}).catch(function(){});}
document.getElementById('tDev').addEventListener('click',function(e){var el=e.target;while(el&&el!==this){if(el.tagName==='BUTTON'&&el.getAttribute('data-acc')){var ip=el.getAttribute('data-ip');var acc=el.getAttribute('data-acc');if(acc==='block'){var mot=prompt('motivo del bloqueo (opcional):','');if(mot===null)return;fetch('/api/admin/dispositivo/toggle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ip:ip,bloquear:true,motivo:mot,token:tok})}).then(function(){mostrarToast('✅ dispositivo bloqueado');cargarDatos();});}else{fetch('/api/admin/dispositivo/toggle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ip:ip,bloquear:false,motivo:'',token:tok})}).then(function(){mostrarToast('✅ dispositivo desbloqueado');cargarDatos();});}return;}el=el.parentElement;}});
...
# ============================================================
# SERVIDOR HTTP (IGUAL QUE ANTES - NO CAMBIA)
# ============================================================
...
def main():
    ...
    try:
        ...
        for linea in resultado.stdout.split('\n'):
            if "IPv4" in linea and "192.168.137" in linea:
                ip_wifi = linea.split(":")[-1].strip(); break
    except: ip_wifi = ip_local
    print("\n" + "=" * 70)
    ...
    try: servidor.serve_forever()
        ...
        print("\n⏹️ Servidor detenido.")
        db.log_evento('INFO', 'Servidor detenido')
        servidor.shutdown()
