...
# ============================================================
# AUTOARRANQUE Y MODO SILENCIOSO
# ============================================================
APP_NAME = "MiPakete"
...
def instalar_autoarranque():
    ...
    try:
        ...
        if getattr(sys, 'frozen', False):
            comando = f'"{ruta}" --silent'
        else:
            comando = f'pythonw "{ruta}" --silent'
        ...
        winreg.CloseKey(key)
        print(f"  ✅ Autoarranque instalado")
        print(f"     El servidor iniciara automaticamente con Windows")
        print(f"     Registro: HKCU\\...\\Run\\{APP_NAME}")
        return True
...
def desinstalar_autoarranque():
    ...
    try:
        ...
        try:
            ...
            print(f"  ✅ Autoarranque desinstalado")
            winreg.CloseKey(key)
            return True
        except FileNotFoundError:
            print(f"  ℹ️ No estaba instalado")
            winreg.CloseKey(key)
            return True
    except Exception as e:
        print(f"  ❌ Error: {e}")
...
# ============================================================
# HTML PAGINA PRINCIPAL (CORREGIDA)
# ============================================================
...
.pwd{-webkit-text-security:disc;text-security:disc;letter-spacing:2px}
.pwd::placeholder{-webkit-text-security:none;text-security:none;letter-spacing:normal}
...
<div class="fg"><label>contrasena</label><input type="text" name="password" class="pwd" autocomplete="new-password" autocapitalize="off" spellcheck="false" required></div>
...
body{font-family:'Segoe UI',Arial,sans-serif;background:#050816;color:#e0e6ed;display:flex;align-items:center;justify-content:center;min-height:100vh;min-height:100dvh;position:relative}
...
.modal{position:relative;z-index:2;background:rgba(10,14,39,0.85);backdrop-filter:blur(20px);border:1px solid rgba(255,51,102,0.3);border-radius:24px;padding:40px 32px;max-width:460px;width:92%;text-align:center;box-shadow:0 20px 80px rgba(0,0,0,0.7),0 0 60px rgba(255,51,102,0.15);animation:pop 0.5s cubic-bezier(0.34,1.56,0.64,1);overflow:hidden;max-height:95vh;overflow-y:auto}
.modal::before{content:'';position:absolute;top:-1px;left:0;right:0;height:3px;background:linear-gradient(90deg,#ff3366,#ffaa00,#ff3366);border-radius:24px 24px 0 0}
@keyframes pop{from{opacity:0;transform:scale(0.8) translateY(30px)}to{opacity:1;transform:scale(1) translateY(0)}}
.icon{width:90px;height:90px;margin:0 auto 20px;border-radius:50%;background:radial-gradient(circle,rgba(255,51,102,0.25),rgba(255,51,102,0.05));border:2px solid rgba(255,51,102,0.4);display:flex;align-items:center;justify-content:center;font-size:46px;animation:shake 2s ease-in-out infinite}
@keyframes shake{0%,100%{transform:rotate(0)}25%{transform:rotate(-5deg)}75%{transform:rotate(5deg)}}
.modal h1{font-size:24px;color:#ff3366;margin-bottom:10px;font-family:'Courier New',monospace;font-weight:700}
...
.plan-box h3{color:#00ff88;font-family:'Courier New',monospace;font-size:13px;margin-bottom:12px;text-transform:uppercase;letter-spacing:1px}
.plan{display:flex;justify-content:space-between;align-items:center;padding:8px 0;font-family:'Courier New',monospace;font-size:13px;color:rgba(224,230,237,0.8)}
.plan strong{color:#00ff88}
.fg{margin-bottom:14px;text-align:left}
.fg label{display:block;margin-bottom:6px;color:rgba(224,230,237,0.6);font-size:11px;font-weight:600;font-family:'Courier New',monospace;text-transform:uppercase;letter-spacing:0.5px}
.fg input{width:100%;padding:12px 14px;background:rgba(0,0,0,0.4);border:1px solid rgba(0,255,136,0.25);border-radius:10px;color:#e0e6ed;font-family:'Courier New',monospace;font-size:14px;text-transform:uppercase;letter-spacing:2px;text-align:center;transition:all 0.3s}
.fg input:focus{outline:none;border-color:#00ff88;box-shadow:0 0 0 3px rgba(0,255,136,0.15)}
.bt{padding:13px 22px;border-radius:10px;border:none;font-weight:700;font-size:14px;cursor:pointer;transition:all 0.3s;font-family:'Courier New',monospace;width:100%;margin-top:8px}
.bp{background:linear-gradient(135deg,#00ff88,#00d4ff);color:#050816;box-shadow:0 4px 20px rgba(0,255,136,0.3)}
.bp:hover{transform:translateY(-2px);box-shadow:0 8px 30px rgba(0,255,136,0.5)}
.bp:disabled{opacity:0.5;cursor:not-allowed}
.bg{background:transparent;color:rgba(224,230,237,0.5);border:1px solid rgba(224,230,237,0.15);margin-top:10px;font-size:12px}
.bg:hover{color:#e0e6ed;border-color:rgba(224,230,237,0.4)}
.toast{position:fixed;bottom:24px;left:50%;transform:translateX(-50%) translateY(100px);background:rgba(10,14,39,0.95);backdrop-filter:blur(15px);border:1px solid #00ff88;border-radius:12px;padding:14px 22px;font-family:'Courier New',monospace;font-size:13px;color:#00ff88;z-index:9999;box-shadow:0 8px 30px rgba(0,0,0,0.5);opacity:0;transition:all 0.4s}
...
<p class="msg">Tu dispositivo tiene las descargas <strong style="color:#ff3366">desactivadas</strong>.<br>Canjea un codigo o contacta al administrador para reactivarlas.</p>
<div class="plan-box">
...
var ch='01アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヲン';
var fs=14,cl=Math.floor(cv.width/fs),dr=[];
for(var i=0;i<cl;i++)dr[i]=1;
setInterval(function(){
cx.fillStyle='rgba(5,8,22,0.05)';cx.fillRect(0,0,cv.width,cv.height);
cx.fillStyle='#ff3366';cx.font=fs+'px monospace';
...
document.getElementById('codInput').addEventListener('keypress',function(e){
if(e.key==='Enter')document.getElementById('btnActivar').click();
});
...
# ============================================================
# HTML PANEL ADMIN (CON SWITCH ANDROID)
# ============================================================
...
.slider:before{position:absolute;content:"";height:22px;width:22px;left:3px;bottom:2px;background:#fff;transition:.35s;border-radius:50%;box-shadow:0 2px 8px rgba(0,0,0,.5)}
input:checked+.slider{background:linear-gradient(135deg,var(--p),var(--s));border-color:var(--p);box-shadow:0 0 16px rgba(0,255,136,.5)}
input:checked+.slider:before{transform:translateX(24px)}
.estTxt{font-size:11px;font-weight:700;margin-top:6px;letter-spacing:0.5px;text-align:center}
.estTxt.on{color:var(--p)}
.estTxt.off{color:var(--d)}
...
<div class="fg"><label>Nueva contrasena</label><input type="text" id="cfgPass" class="pwd" placeholder="nueva" autocomplete="new-password" autocapitalize="off" spellcheck="false"></div>
<div class="fg"><label>Confirmar</label><input type="text" id="cfgPass2" class="pwd" placeholder="confirmar" autocomplete="new-password" autocapitalize="off" spellcheck="false"></div>
</div>
...
var activo=dv2.bloqueado!==1;
var ipId=escA(dv2.ip).replace(/\./g,'-');
dh+='<label class="switch" title="'+(activo?'descargas activas':'descargas bloqueadas')+'">';
dh+='<input type="checkbox" data-ip="'+escA(dv2.ip)+'" data-acc="switch" '+(activo?'checked':'')+'>';
dh+='<span class="slider"></span></label>';
dh+='<div class="estTxt '+(activo?'on':'off')+'" id="est-'+ipId+'">'+(activo?'ON':'OFF')+'</div>';
dh+='</td></tr>';}
...
var permitir=el.checked;
var ipId=ip.replace(/\./g,'-');
var est=document.getElementById('est-'+ipId);
var motivo='';
if(!permitir){
motivo=prompt('motivo del bloqueo (opcional):','');
if(motivo===null){el.checked=false;return;}
}
fetch('/api/admin/dispositivo/toggle',{method:'POST',headers:{'Content-Type':'application/json'},
body:JSON.stringify({ip:ip,bloquear:!permitir,motivo:motivo,token:tok})})
.then(function(r){
if(!r.ok)throw new Error();
if(est){est.textContent=permitir?'ON':'OFF';est.className='estTxt '+(permitir?'on':'off');}
mostrarToast(permitir?'✅ descargas activadas para '+ip:'⛔ descargas bloqueadas para '+ip,!permitir);
setTimeout(cargarDatos,500);
})
.catch(function(){el.checked=!permitir;mostrarToast('❌ error al cambiar estado',true);});
return;
}
if(el.tagName==='BUTTON'&&el.getAttribute('data-acc')){
var ip2=el.getAttribute('data-ip');
if(el.getAttribute('data-acc')==='nombre'){
var nom=prompt('nombre del dueño del dispositivo '+ip2+':',el.getAttribute('data-nombre')||'');
if(nom===null)return;
fetch('/api/admin/dispositivo/nombre',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ip:ip2,nombre:nom,token:tok})}).then(function(){mostrarToast('✅ nombre guardado');cargarDatos();});
}
return;
}
el=el.parentElement;}});
...
def main():
    ...
    # Modo instalar/desinstalar y salir
    if args.install:
    ...
    # Modo silencioso: ocultar consola
    if args.silent:
        ocultar_consola()
    ...
    # Verificar estado del autoarranque
